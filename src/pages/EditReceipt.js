...
import { calculateTotal, getReceiptById, updateReceipt, formatDate, formatTime } from '../utils/receiptUtils';
...
const EditReceipt = () => {
  ...
  useEffect(() => {
    ...
    getReceiptById(id)
      .then(receiptData => {
        ...
        setItems(receiptData.items || [{ name: '', price: '', quantity: '1', costPrice: '0', quantityUnit: 'units' }]);
        setOriginalItems([...receiptData.items]); // Store original items for inventory adjustment
        ...
        setTransactionId(receiptData.transactionId || '');
      })
      .catch(error => {
  ...
  const validateInventory = () => {
    ...
    for (const item of items) {
      // Skip validation for items that were already in the receipt with the same quantity
      const originalItem = originalItems.find(origItem => 
        origItem.name.toLowerCase() === item.name.toLowerCase());
      
      if (originalItem && parseFloat(originalItem.quantity) >= parseFloat(item.quantity)) {
        // If item was already in receipt with same or higher quantity, no need to validate
        continue;
      }
      
      // For new items or increased quantities, check inventory
      ...
      if (!matchingStock) {
        ...
        let additionalQty = parseFloat(item.quantity);
        if (originalItem) {
          additionalQty -= parseFloat(originalItem.quantity);
        }
        
        if (additionalQty > 0 && matchingStock.quantity < additionalQty) {
          invalidItems.push({
            name: item.name,
  ...
  const calculateInventoryAdjustments = () => {
    // For each item in the updated receipt, compare with original receipt
    const adjustments = [];
    
    // First, handle removed items (add back to inventory)
    originalItems.forEach(origItem => {
      const stillExists = items.some(item => item.name === origItem.name);
      if (!stillExists) {
        // Item was removed, add back to inventory
        adjustments.push({
          name: origItem.name,
          quantity: -parseFloat(origItem.quantity), // Negative to add back to inventory
          quantityUnit: origItem.quantityUnit || 'units'
        });
      }
    });
    
    // Then handle new or modified items
    items.forEach(item => {
      const origItem = originalItems.find(oi => oi.name === item.name);
      if (!origItem) {
        // New item, deduct from inventory
        adjustments.push({
          name: item.name,
        ...
        // Modified item, calculate difference
        const qtyDiff = parseFloat(item.quantity) - parseFloat(origItem.quantity);
        if (qtyDiff !== 0) {
          adjustments.push({
            name: item.name,
            quantity: qtyDiff,
  ...
  const handleSubmit = (e) => {
    ...
    const receiptData = {
      ...
      shopDetails: originalReceipt.shopDetails,
      transactionId,
      ...
      paymentMethod,
      timestamp: originalReceipt.timestamp // Keep original timestamp
    };
    
    // Update receipt in Firestore
    updateReceipt(id, receiptData)
      .then(() => {
        setSuccess(<Translate textKey="receiptUpdated" fallback="Receipt updated successfully" />);
        
        // Calculate and apply inventory adjustments
        const inventoryAdjustments = calculateInventoryAdjustments();
        if (inventoryAdjustments.length > 0) {
          return updateStockQuantity(currentUser.uid, inventoryAdjustments);
        }
      })
      .catch(error => {
        setError(<><Translate textKey="errorUpdatingReceipt" fallback="Error updating receipt" /> {error.message}</>);
      })
      .finally(() => {
        setLoading(false);
      });
    ...
    return (
      <>
        ...
        <Container className="text-center mt-5">
          <p><Translate textKey="loadingReceipt" fallback="Loading receipt..." /></p>
        </Container>
      </>
    );
  }
    ...
    return (
      <>
        ...
        <Container className="mt-4">
          ...
          >
            <Translate textKey="backToReceipts" fallback="Back to Receipts" />
          </Button>
        </Container>
      </>
    );
  ...
  return (
    <>
      <MainNavbar />
      <Container>
        <h2 className="mb-4"><Translate textKey="editReceipt" fallback="Edit Receipt" /></h2>
                ...
                <Form onSubmit={handleSubmit}>
                  ...
                  <div className="d-flex mt-4">
                    <Button 
                      ...
                      className="me-2"
                    >
                      <Translate textKey="updateReceipt" fallback="Update Receipt" />
                    </Button>
                ...
                <div className="receipt-preview">
                  ...
                  <Row className="mb-3">
                    <Col xs={6}>
                      ...
                      <p className="mb-1"><strong><Translate textKey="date" /></strong> {originalReceipt ? formatDate(originalReceipt.timestamp) : new Date().toLocaleDateString()}</p>
                      <p className="mb-1"><strong><Translate textKey="time" /></strong> {originalReceipt ? formatTime(originalReceipt.timestamp) : new Date().toLocaleTimeString()}</p>
                    </Col>
                    <Col xs={6}>
                  ...
                  <Table borderless className="receipt-table">
                    ...
                    <tfoot>
                      <tr>
                        ...
                        <th className="text-end">RS {calculateTotal(items)}</th>
                      </tr>
                    </tfoot>
                  </Table>
                  ...
                  <div className="text-center mt-4">
                    ...
                    <p className="small text-muted">Receipt ID: {id || 'N/A'}</p>
                  </div>
                </div>
              </Card.Body>
            </Card>
